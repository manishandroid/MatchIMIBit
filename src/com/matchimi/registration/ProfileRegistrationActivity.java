...
//	private EditText phoneNumber;
	...
	private final String dobFormat = "yyyy-MM-dd";
	private final SimpleDateFormat facebookDateFormat = new SimpleDateFormat("MM/dd/yyyy", Locale.US);
...
//		photoBackNRIC.setOnClickListener(backNRICListener);
		...
		if (bundleExtras != null) {
			...
			fname = bundleExtras
					.getString(CommonUtilities.USER_FULLNAME);
...
//			}
			...
			Log.d(CommonUtilities.TAG, "Facebook DOB " + bday);
			
			if (bday.length() == 10) {
				try {
					Date birthdayFromFacebook = facebookDateFormat.parse(bday);
					
					Log.d(CommonUtilities.TAG, "Facebook DOB " + birthdayFromFacebook);
			...
			for (int i = 0; i < listGender.size(); i++) {
				...
				rb.setOnCheckedChangeListener(new OnCheckedChangeListener() {
					...
					public void onCheckedChanged(CompoundButton arg0,
						...
						if (arg1) {
							for(int i=0; i < listGender.size(); i++) {
								if(listGender.get(i).equals(arg0.getText().toString())) {
									genderSelected = listGenderId.get(i);						
								}
							}
						}
					}
				...
				if (getIntent().hasExtra(CommonUtilities.USER_GENDER) &&
						...
						!b.getString(CommonUtilities.USER_GENDER).equals("null")) {
					rb.setEnabled(false);
				} else {
					rb.setEnabled(true);
				}
...
//			}
	...
	protected void doCreateProfile() {
		final String url = SERVERURL + API_CREATE_AND_PART_TIMER_REGISTRATION;		
		final Handler mHandlerFeed = new Handler();
		final Runnable mUpdateResultsFeed = new Runnable() {
			public void run() {
				...
				if (jsonStr != null) {
					...
					if (jsonStr.trim().equalsIgnoreCase("0") || 
						...
						editor.putString(USER_FULLNAME, fname);
