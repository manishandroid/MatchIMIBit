...
public class HomeActivity extends TabSwipeActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (settings == null
			...
			String facebookID = settings.getString(CommonUtilities.USER_FACEBOOK_ID, null);
			if(facebookID != null && facebookID.length() > 0) {
				Log.d(CommonUtilities.TAG, "Activate facebook friends periodic task");
				setAlarmManager();			
			}
	...
	private void loadStaticData() {
		...
		final String url = CommonUtilities.SERVERURL + CommonUtilities.API_GET_STATIC_DATA;
		...
		progress = ProgressDialog.show(context, getString(R.string.preparing_data), 
	...
	private void saveGender(String jsonStr) {
		try {
			...
			JSONArray jsonArray = dataObj.getJSONArray(CommonUtilities.PARAM_STATIC_GENDERS);
			
			editor = settings.edit();
			editor.putString(CommonUtilities.API_CACHE_GENDERS, jsonArray.toString());
			editor.commit();
									
			saveNRICType(jsonStr);
			
		} catch (JSONException e) {
			NetworkUtils.connectionHandler(HomeActivity.this, jsonStr, e.getMessage());
	...
	private void saveNRICType(String jsonStr) {
		...
		try {
			JSONObject dataObj = new JSONObject(jsonStr);
			JSONArray jsonArray = dataObj.getJSONArray(CommonUtilities.PARAM_STATIC_NRIC_TYPES);
			
			editor = settings.edit();
			editor.putString(CommonUtilities.API_CACHE_NRIC_TYPES, jsonArray.toString());
			editor.commit();
	...
	private void saveSchool(String jsonStr) {
		try {
			JSONObject dataObj = new JSONObject(jsonStr);
			JSONArray jsonArray = dataObj.getJSONArray(CommonUtilities.PARAM_STATIC_SCHOOLS);
			
			editor = settings.edit();
			editor.putString(CommonUtilities.API_CACHE_SCHOOLS, jsonArray.toString());
			editor.commit();
	...
	private void saveLocation(String jsonStr) {
		try {
			JSONObject dataObj = new JSONObject(jsonStr);
			JSONArray jsonArray = dataObj.getJSONArray(CommonUtilities.PARAM_STATIC_LOCATIONS);
			
			editor = settings.edit();
			editor.putString(CommonUtilities.API_CACHE_LOCATIONS, jsonArray.toString());
			editor.commit();
	...
	private void saveDays(String jsonStr) {
		try {
			JSONObject dataObj = new JSONObject(jsonStr);
			JSONArray jsonArray = dataObj.getJSONArray(CommonUtilities.PARAM_STATIC_DAYS);
			
			editor = settings.edit();
			editor.putString(CommonUtilities.API_CACHE_DAYS, jsonArray.toString());
			editor.commit();
	...
	private void saveJobFunctions(String jsonStr) {
		try {
			JSONObject dataObj = new JSONObject(jsonStr);
			JSONArray jsonArray = dataObj.getJSONArray(CommonUtilities.PARAM_STATIC_JOB_FUNCTIONS);
			
			editor = settings.edit();
			editor.putString(CommonUtilities.API_CACHE_JOB_FUNCTIONS, jsonArray.toString());
			editor.commit();
	...
	private void setAlarmManager() {
		...
		am.setRepeating(AlarmManager.RTC_WAKEUP, calendar.getTimeInMillis(), 1000 * 60 * 60, pi);
